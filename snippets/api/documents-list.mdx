...
<pre><code className="language-bash">{props.exTraceEvents}</code></pre>

Con la flag activa, cada documento de la respuesta incluye un array `traces`, y cada traza un array `events` (ver el schema `Trace` y `TraceEvent` en la referencia). El endpoint de detalle (<code>{props.exDetailPath}</code>) ya retorna estos eventos de forma permanente, sin necesidad de la flag.
...
<pre><code className="language-bash">{props.exBookMetadata}</code></pre>
...
<pre><code className="language-bash">{props.exJsonParam}</code></pre>

Esto devuelve solo los documentos donde `json_param` contiene esa key con exactamente ese valor (coincidencia exacta, no búsqueda parcial). Puedes combinar varias keys en la misma request — se aplican con AND:

<pre><code className="language-bash">{props.exJsonParamAnd}</code></pre>
