...
Por defecto (`auto_issue=true`), Tu Pana emite el DTE automáticamente al confirmarse el pago. Si necesitas controlar el momento de la emisión, envía `auto_issue=false` y emite el DTE cuando lo necesites con [`POST /v1/payment-requests/{id}/issue/`](/api-reference/payment-requests/issue).
